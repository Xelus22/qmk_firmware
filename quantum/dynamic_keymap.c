...
void dynamic_keymap_macro_get_repeat_data(uint8_t id, uint8_t *data) {
    ...
    void    *source = (void *)(DYNAMIC_KEYMAP_MACRO_REPEAT_EEPROM_ADDR + id * sizeof(dynamic_macro_header_t));
    uint8_t *target = data;
    for (uint16_t i = 0; i < sizeof(dynamic_macro_header_t); i++) {
        if (i < DYNAMIC_KEYMAP_MACRO_EEPROM_SIZE) {
            *target = eeprom_read_byte(source);
        } else {
            *target = 0x00;
        }
...
void dynamic_keymap_macro_set_repeat_data(uint8_t id, uint8_t *data) {
    if (id >= DYNAMIC_KEYMAP_MACRO_COUNT) {
        return;
    }
    ...
    uint8_t *source = data;
    for (uint16_t i = 0; i < sizeof(dynamic_macro_header_t); i++) {
        if (i < DYNAMIC_KEYMAP_MACRO_EEPROM_ADDR) {
            eeprom_update_byte(target, *source);
        }
        source++;
        target++;
...
uint32_t dynamic_keymap_macro_get_repeat_interval(uint8_t id) {
    ...
    uint32_t interval = eeprom_read_dword(source) & 0x00FFFFFF;
    return interval;
}
#endif
