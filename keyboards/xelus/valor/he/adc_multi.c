...
#if STM32_ADC_USE_ADC1
        if (&ADCD1 == adcp) rccDisableADC1();
#endif

#if STM32_ADC_USE_ADC2
        if (&ADCD2 == adcp) rccDisableADC2();
#endif

#if STM32_ADC_USE_ADC3
        if (&ADCD3 == adcp) rccDisableADC3();
#endif
    }
}
...
void adc_multi_lld_start_conversion(ADCDriver *adcp) {
    ...
    cr2 = grpp->cr2 | ADC_CR2_CONT | ADC_CR2_ADON;
...
void adcMultiStart(void) {
    ...
    chSysLock();
    osalDbgAssert((adcp->state == ADC_STOP) || (adcp->state == ADC_READY), "invalid state");
    adc_multi_lld_start();
 ...
 * @iclass
 */
void adcMultiStartConversionI(ADCDriver *adcp, const ADCConversionGroup *grpp, adcsample_t *samples, size_t depth) {
    osalDbgCheckClassI();
    osalDbgCheck((adcp != NULL) && (grpp != NULL) && (samples != NULL) && ((depth == 1U) || ((depth & 1U) == 0U)));
    osalDbgAssert((adcp->state == ADC_READY) || (adcp->state == ADC_COMPLETE) || (adcp->state == ADC_ERROR), "not ready");
 ...
 * @api
 */
void adcMultiStartConversion(const ADCConversionGroup *grpp1, const ADCConversionGroup *grpp2, const ADCConversionGroup *grpp3, adcsample_t *samples, size_t depth) {
    chSysLock();
    // Multi-DMA mode tuning
    ADC->CCR |= ADC_CCR_DMA_0 | ADC_CCR_DDS | (ADC_CCR_MULTI_TRIPLE | ADC_CCR_MULTI_REGULAR_SIMUL);
    // DMA Mode 1
